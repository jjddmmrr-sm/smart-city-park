...
export const Route = createFileRoute("/admin/fine-types")({
  component: AdminFineTypesPage,
  ssr: false,
});
...
function AdminFineTypesPage() {
  ...
  const openCreate = () => {
    setEditing(null);
    setForm({ code: "", name: "", description: "", amount: "20", status: "active" });
    setShowForm(true);
  };

  const openEdit = (row: FineType) => {
    setEditing(row);
    setForm({
      code: row.code,
      name: row.name,
  ...
  const save = async () => {
    ...
    if (editing) await apiPatchAuth(`/parking/fine-types/${editing.id}`, payload);
    else await apiPostAuth("/parking/fine-types", payload);
  ...
  const toggleStatus = async (row: FineType) => {
    await apiPatchAuth(`/parking/fine-types/${row.id}`, {
  ...
  return (
    <div className="h-full overflow-auto bg-surface">
      <div className="px-4 py-4 space-y-4">
        ...
        <Panel title="Tipos registrados" padded={false}>
