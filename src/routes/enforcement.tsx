    ...
    <div className="h-full flex flex-col bg-surface">
      <div className="px-4 py-3 border-b border-border bg-card">
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
          <KpiTile label="Casos totales" value={fmtInt(counts.total)} sub="datos reales" icon={<FileWarning className="h-4 w-4" />} accent="primary" />
          ...
          <KpiTile label="Estado" value={loading ? "..." : "Online"} sub="/frontend/enforcement" icon={<ShieldCheck className="h-4 w-4" />} accent="success" />
        </div>
      </div>
      ...
      <div className="px-4 py-2 border-b border-border bg-card flex items-center gap-2 flex-wrap">
        ...
        <Sel value={zone} onChange={setZone} options={[{ v: "all", l: "Todas las zonas" }, ...zones.map((z) => ({ v: z.id, l: z.name }))]} />
              ...
              <tbody>
                {rows.slice(0, 500).map((c, i) => (
                  <tr key={c.id} onClick={() => setSelected(c)} className={"border-t border-border hover:bg-surface-2 cursor-pointer " + (i % 2 ? "bg-surface-2/40" : "")}>
                    ...
                    <td className="px-3 py-1.5"><StatusPill status={statusEs(c.priority)} /></td>
          ...
          {selected ? (
            <div className="space-y-2 text-[12px]">
              <div className="flex items-center justify-between">
                <span className="text-[14px] font-mono font-semibold">{friendlyId(selected.id)}</span>
              ...
              <div className="grid grid-cols-1 gap-2 pt-1">
                <button onClick={() => updateStatus(selected.id, "reviewing")} className="h-8 text-[12px] rounded-md border border-border hover:bg-secondary">
                  Marcar en revisión
                </button>
                <button onClick={() => updateStatus(selected.id, "fined")} className="h-8 text-[12px] rounded-md bg-primary text-primary-foreground hover:bg-primary/90">
                  Emitir multa
                </button>
                <button onClick={() => updateStatus(selected.id, "resolved")} className="h-8 text-[12px] rounded-md border border-border hover:bg-secondary">
                  Resolver caso
                </button>
              </div>
            </div>
          ) : (
            <div className="text-[12px] text-muted-foreground">Haga clic en cualquier caso para inspeccionar el detalle.</div>
          )}
        </Panel>
      </div>
    </div>
