    ...
    <div className="h-full overflow-auto bg-surface">
      <div className="px-4 py-4 space-y-4">
        <div>
          ...
          <p className="text-[12px] text-muted-foreground">Estado real de parámetros, inventario y módulos conectados a PostgreSQL.</p>
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
          <Panel title="Zonas y tarifas reales" action={<span className="text-[10px] text-muted-foreground inline-flex items-center gap-1"><Building2 className="h-3 w-3" /> {zones.length} zonas</span>} padded={false}>
              ...
              <tbody>
                {zones.map((z) => (
                  <tr key={z.zone_id} className="border-t border-border">
                    <td className="px-3 py-2"><div className="font-medium">{z.zone_name}</div><div className="text-[10px] text-muted-foreground font-mono">{z.zone_id.slice(0, 8).toUpperCase()}</div></td>
          ...
          <Panel title="Inventario real por tipo de espacio" action={<span className="text-[10px] text-muted-foreground inline-flex items-center gap-1"><ParkingSquare className="h-3 w-3" /> {fmtInt(spaces.length)} espacios</span>} padded={false}>
            <table className="w-full text-[12px]">
              <thead className="bg-surface-2 text-[10px] uppercase tracking-wider text-muted-foreground">
                <tr>
                  <Th>Zona</Th>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
          <Panel title="Módulos conectados" action={<span className="text-[10px] text-muted-foreground inline-flex items-center gap-1"><Bell className="h-3 w-3" /> {modules.length} endpoints</span>} padded={false}>
            <table className="w-full text-[12px]">
              <thead className="bg-surface-2 text-[10px] uppercase tracking-wider text-muted-foreground">
                <tr><Th>Módulo</Th><Th>Endpoint</Th><Th>Estado</Th></tr>
              </thead>
              <tbody>
                {modules.map((m) => (
          ...
          <Panel title="Controladores reales" action={<span className="text-[10px] text-muted-foreground inline-flex items-center gap-1"><UsersRound className="h-3 w-3" /> {controllers.length} controladores</span>} padded={false}>
            <table className="w-full text-[12px]">
              <thead className="bg-surface-2 text-[10px] uppercase tracking-wider text-muted-foreground">
                <tr><Th>Nombre</Th><Th>Zona</Th><Th>Turno</Th><Th>Estado</Th></tr>
              </thead>
              <tbody>
                {controllers.map((c) => (
                  <tr key={c.controlador_id} className="border-t border-border">
                    <td className="px-3 py-2"><div className="font-medium">{c.nombre}</div><div className="text-[10px] text-muted-foreground font-mono">CTRL-{c.controlador_id.slice(0, 8).toUpperCase()}</div></td>
                    <td className="px-3 py-2 text-muted-foreground">{c.zona_asignada}</td>
                    <td className="px-3 py-2 capitalize">{c.turno}</td>
                    <td className="px-3 py-2"><StatusPill status={estadoEs(c.estado)} /></td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          ...
          <Panel title="Operación actual">
            <div className="space-y-2 text-[12px]">
              <Row k="Espacios totales" v={fmtInt(overview?.totalSpaces ?? spaces.length)} icon={<ParkingSquare className="h-3.5 w-3.5" />} />
          ...
          <Panel title="Seguridad y gobierno">
            <div className="space-y-2 text-[12px]">
              <Row k="JWT Backend" v={<StatusPill status="activo" />} icon={<ShieldAlert className="h-3.5 w-3.5" />} />
