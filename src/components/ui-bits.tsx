...
export function KpiTile({
  ...
  return (
    <div className="relative bg-card border border-border rounded px-2.5 py-1.5 flex items-center gap-2 min-w-0">
      <span
        className={
          "absolute left-0 top-1.5 bottom-1.5 w-[3px] rounded-r " + accentBar[accent ?? "primary"]
        }
      />
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="text-[15px] sm:text-[16px] font-semibold tabular-nums leading-tight text-foreground truncate">
          {value}
        </div>
        {sub && (
          <div className="text-[10px] text-muted-foreground tabular-nums truncate">{sub}</div>
        )}
      </div>
    </div>
  );
    ...
    <section className={"bg-card border border-border rounded flex flex-col min-h-0 " + className}>
      {title && (
        <header className="flex items-center justify-between px-3 h-9 border-b border-border">
          <h3 className="text-[11px] font-semibold uppercase tracking-wider text-primary truncate">
            {title}
          </h3>
          {action}
        </header>
...
export function StatusPill({ status }: { status: string }) {
  ...
  const dotCls = ["available", "paid", "valid", "resolved", "pagada", "activo"].includes(status)
    ? "bg-success"
    : ["occupied", "unpaid", "no_payment", "high", "alta"].includes(status)
      ? "bg-destructive"
      : [
            "reserved",
            "partial",
        ...
        ? "bg-warning"
        : ["reviewing", "apelada"].includes(status)
          ? "bg-info"
          : status === "fined"
            ? "bg-primary"
            : "bg-muted-foreground";
  ...
  return (
    <span
      className={
        "inline-flex items-center gap-1.5 px-2 h-5 rounded text-[11px] font-medium " +
