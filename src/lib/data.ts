...
export type Multa = {
  multa_id: string; fecha_hora: string; placa: string; zona: string; calle: string;
  motivo_codigo: "sin_pago" | "exceso_tiempo" | "zona_prohibida" | "zona_reservada";
  motivo_descripcion: string; valor_multa_usd: number; controlador_id: string;
...
export type MultaResumen = {
  ...
  multas_sin_pago: number; multas_exceso_tiempo: number;
  multas_zona_prohibida: number; multas_zona_reservada: number;
  total_multas: number; monto_total_usd: number;
};
export type Controlador = {
  controlador_id: string; nombre: string; zona_asignada: string;
  turno: "mañana" | "tarde" | "mixto";
  estado: "activo" | "vacaciones" | "inactivo";
  fecha_ingreso: string; dispositivo_id: string;
};
export type ControladorProd = {
  fecha: string; controlador_id: string; zona: string;
  inspecciones_realizadas: number; vehiculos_validados: number;
  alertas_emitidas: number; multas_generadas: number;
  tiempo_promedio_respuesta_min: number;
  porcentaje_casos_cerrados: number; recorrido_km: number;
  turno: string;
};
export type MedioPago = {
  ...
  transacciones: number; monto_total_usd: number;
  tickets_promedio_usd: number; porcentaje_participacion: number;
};
...
export const CONTROLADOR_BY_ID = Object.fromEntries(CONTROLADORES.map((c) => [c.controlador_id, c]));
